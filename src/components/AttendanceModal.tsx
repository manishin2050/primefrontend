...
export const AttendanceModal: React.FC<AttendanceModalProps> = ({ session, isOpen, onClose }) => {
  ...
  useEffect(() => {
    if (attendancesData?.data?.attendances) {
      const initial: Record<number, AttendanceStatus> = {};
      attendancesData.data.attendances.forEach((att: any) => {
        initial[att.studentId] = att.status as AttendanceStatus;
      });
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg shadow-xl max-w-4xl w-full mx-4 max-h-[90vh] overflow-hidden flex flex-col">
              ...
              {students.map((enrollment: any) => (
                <div
                  ...
                  <div className="flex items-center space-x-2">
                    ...
                    {/* Present */}
                    <button
                      onClick={() => handleAttendanceChange(enrollment.studentId, AttendanceStatus.PRESENT)}
                      disabled={isMarking}
                    ...
                    {/* Manual */}
                    <button
                      onClick={() => handleAttendanceChange(enrollment.studentId, AttendanceStatus.MANUAL_PRESENT, true)}
                      disabled={isMarking}
                      className={`px-3 py-1 text-sm rounded-md transition ${
                    ...
                    {/* Absent */}
                    <button
                      onClick={() => handleAttendanceChange(enrollment.studentId, AttendanceStatus.ABSENT)}
                      disabled={isMarking}
                      className={`px-3 py-1 text-sm rounded-md transition ${
        ...
        <div className="px-6 py-4 border-t border-gray-200 flex justify-end space-x-3">
          <button onClick={onClose} className="px-4 py-2 text-sm font-medium text-gray-700 bg-white border rounded-md">
            Close
          </button>
          <button
            onClick={handleBulkSave}
            disabled={isMarking}
            className="px-4 py-2 text-sm font-medium text-white bg-orange-600 rounded-md disabled:opacity-50">
